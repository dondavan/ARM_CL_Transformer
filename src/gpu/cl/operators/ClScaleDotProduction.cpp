                                     ...
                                     const ScaleDotProductionAttentionLayerInfo &info)
{
    ...
    if(query->data_type() == DataType::F32) std::cout << "wo cao ni ma de query type 1" << std::endl;
    auto softmax_kernel = std::make_unique<kernels::ClSoftmaxKernel>();
    if(query->data_type() == DataType::F32) std::cout << "wo cao ni ma de query type 2" << std::endl;
