...
void NEPositionEmbeddingLayer::configure(ITensor *input, ITensor *position, ITensor *output)
{
    ...
    std::cout.percesion(5) << std::scientific << "NEPositionEmbeddingLayer::configure cost: " << cost_time << std::endl;
#endif
}
...
void NEPositionEmbeddingLayer::run()
{
    ...
    auto   end_time  = std::chrono::high_resolution_clock::now();
    double cost_time = std::chrono::duration_cast<std::chrono::duration<double>>(end_time - start_time).count();
    std::cout.percesion(5) << std::scientific << "NEPositionEmbeddingLayer::run cost: " << cost_time << std::endl;
#endif
}
